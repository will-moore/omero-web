...
from omero.plugins.web import APACHE_MOD_WSGI_ERR, WebControl
...
class TestWeb(object):
    ...
    def add_hostport(self, host, port, monkeypatch):
        if host:
            monkeypatch.setattr(
                settings, "APPLICATION_SERVER_HOST", host, raising=False
            )
        if port:
            monkeypatch.setattr(
                settings, "APPLICATION_SERVER_PORT", port, raising=False
            )
    ...
    def testWebStale(self, app_server, monkeypatch, capsys):
        ...
        stdout = ("Stopping OMERO.web... ",)
        for msg in stdout:
            assert msg in o.split(os.linesep)
        stderr = (
            "[ERROR] OMERO.web workers (PID -999999) - no such process."
    ...
    def testWebWsgiArgs(self, app_server, wsgi_args, monkeypatch, capsys):
        ...
        if wsgi_args is not None:
            startout = (
                "Starting OMERO.web...  `--wsgi-args` is deprecated"
    ...
    def testWebWorkers(self, app_server, wsgi_workers, monkeypatch, capsys):
        ...
        if wsgi_workers is not None:
            startout = (
                "Starting OMERO.web...  `--workers` is deprecated"
    ...
    def testWebWorkerConnections(
        self, app_server, wsgi_worker_conn, monkeypatch, capsys
    ):
        self.mock_django_setting(
            "WSGI_WORKER_CONNECTIONS", wsgi_worker_conn, monkeypatch
        )
        self.mock_django_setting("APPLICATION_SERVER", app_server, monkeypatch)
        self.set_python_dir(monkeypatch)
        self.mock_subprocess_popen(monkeypatch)
        ...
        if wsgi_worker_conn is not None:
            startout = (
                "Starting OMERO.web...  `--worker-connections` is"
    ...
    @pytest.mark.parametrize("server_type", ["nginx", "nginx-development"])
    ...
    def testFullTemplateWithOptions(
        self, server_type, static_root, capsys, monkeypatch
    ):
        prefix = "/test"
        cgihost = "0.0.0.0"
