...
class TestWebGatewayCache(object):
    ...
    def testCacheSettings(self):
        ...
        self.wcache._updateCacheSettings(
            self.wcache._thumb_cache, timeout=2, max_entries=5, max_size=0
        )
        ...
        time.sleep(3)
        assert (
            self.wcache.getThumb(self.request, "test", uid, 0) is None
        ), "Time limit failed"
    ...
    def testThumbCache(self):
        ...
        ), "Thumb not properly cached (%s)" % self.wcache.getThumb(
            self.request, "test", uid, 1
        )
